...
parser.add_argument("--folder", help="上传文件名", default="")
...
def Upload(token,localPath, remotePath, folder, password = ''):
    try:
        upload_header = {
            'UserAgent': UserAgent,
            'Authorization': token,
            'File-Path': parse.quote(f'{remotePath}/{folder}'),
            'Password': password,
...
#        quit()
        time.sleep(2)
        upload_result=Upload(auth_token,args.path,"encrypt_folder",args.folder)
        print(upload_result)
    else:
